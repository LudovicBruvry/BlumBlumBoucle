...
function handleAsteroidLinesTrajectory(dx, dy) {
    ...
    if (ship.deadPoint !== null) {
        const distance = getDistance(ship.x, ship.y, ship.deadPoint.x, ship.deadPoint.y)
        const tickUntilDead = parseInt(distance / spaceSpeed);
        if (ship.nextStepRemainingTick > tickUntilDead) {
            ship.nextStepRemainingTick = tickUntilDead;
            ship.nextPlanetIndex = -1; // means dead
        }
    }
}
...
function calculateShipTrajectory() {
    ...
    ship.nextPlanetIndex = -2; // means void
    ...
    planets.forEach((planet, index) => {
        ...
        if (isIntersectingWithOrbit1 || isIntersectingWithOrbit2) {
            ...
            const anchorPoint = isIntersectingWithOrbit1 ? { x: ptsInnerOrbit[0].x, y: ptsInnerOrbit[0].y } : { x: ptsInnerOrbit[1].x, y: ptsInnerOrbit[1].y };
